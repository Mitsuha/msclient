import 'package:desktop/data/api/tool_auth_api.dart';
import 'package:desktop/domain/tools/claude_initializer.dart';
import 'package:desktop/domain/tools/tool.dart';
import 'package:desktop/domain/tools/tool_initializer.dart';
import 'package:desktop/system/claude_config_manager.dart';

/// The Claude Code tool: `~/.claude` config (credentials store + settings.json)
/// and the `~/.claude.json` profile, plus the `/user/claude-auth` credential
/// endpoint. All orchestration lives in [ConfiguredTool]; this only wires the
/// Claude-specific initializer.
class ClaudeTool extends ConfiguredTool<ClaudeConfigManager> {
  ClaudeTool({
    required super.config,
    required this._authApi,
    required this._resolveProxyUrl,
    required this._requireToken,
  });

  final ToolAuthApi _authApi;
  final ResolveProxyUrl _resolveProxyUrl;
  final RequireToken _requireToken;

  @override
  ToolId get id => ToolId.claude;

  @override
  ToolInitializer initializer({int userPackId = 0}) => claudeInitializer(
    config: config,
    resolveProxyUrl: _resolveProxyUrl,
    fetchAuth: () async => _authApi.createAuth(
      token: await _requireToken(),
      userPackId: userPackId,
    ),
  );
}
