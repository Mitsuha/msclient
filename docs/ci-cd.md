...
用户要求“发布”时，必须执行完整发布流程，不能在提交后停止。只有发布分支和
release tag 都推送成功，才算发布完成；仅要求“提交”时才只创建本地 commit。
...
- 正式发布必须依次完成 `git push github master:rm`、创建 release tag、
  `git push github <tag>`；前一步失败时不得继续，并向用户报告失败原因。
